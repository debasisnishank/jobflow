...
export async function POST(req: NextRequest) {
  try {
    ...
    const { companyDetails, recipientDetails, targetJobTitle, emailType, resumeId } = await req.json();
    ...
    const config = await getAIConfigWithDefaults("email-writer");

    const prompt = ChatPromptTemplate.fromMessages([
      ["system", config.systemPrompt],
      [
        "human",
        `Write a {emailType} email with the following details:

Target Job Title: {targetJobTitle}
Company/Job Details: {companyDetails}
Recipient Information: {recipientDetails}
...
{resumeData}`,
    ...
    const stream = await chain.stream({
      emailType: emailType || "Job Outreach",
      targetJobTitle: targetJobTitle || "",
      companyDetails: companyDetails || "No specific company details provided",
      recipientDetails: recipientDetails || "No recipient details provided",
