...
export async function POST(req: NextRequest) {
  try {
    ...
    const { targetJobTitle, purpose, focusArea, targetAudience, resumeId } = await req.json();
    ...
    const config = await getAIConfigWithDefaults("elevator-pitch");

    const prompt = ChatPromptTemplate.fromMessages([
      ["system", config.systemPrompt],
      [
        "human",
        `Based on this resume, create an elevator pitch:
...
{resumeData}`,
    ...
    const stream = await chain.stream({
      ...
      purpose: purpose || "introducing themselves professionally",
      focusArea: focusArea || "their key strengths and experience",
      targetAudience: targetAudience || "potential employers or networking contacts",
    ...
    return NextResponse.json(
      { error: error.message || "Failed to generate elevator pitch" },
      { status: 500 }
    );
  }
}
