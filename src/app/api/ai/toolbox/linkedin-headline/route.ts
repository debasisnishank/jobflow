...
export async function POST(req: NextRequest) {
  try {
    ...
    const { targetJobTitle, keywords, language, resumeId } = await req.json();
    ...
    const config = await getAIConfigWithDefaults("linkedin-headline");

    const prompt = ChatPromptTemplate.fromMessages([
      ["system", config.systemPrompt],
      [
        "human",
        `Based on this resume, create a LinkedIn headline:
...
{resumeData}`,
    ...
    // Save history and track usage in background after streaming
    const saveHistoryAndTrackUsage = async (output: string) => {
      const { saveAIToolboxHistory } = await import("@/utils/ai-history.utils");
      await saveAIToolboxHistory({
        toolType: "linkedin-headline",
        toolName: "LinkedIn Headline",
        inputData: {
          targetJobTitle,
          keywords,
          language,
      ...
      new ReadableStream({
        async start(controller) {
          try {
            ...
            controller.close();
            // Save history and track usage after streaming completes
            saveHistoryAndTrackUsage(fullOutput).catch(console.error);
          } catch (error) {
    ...
    return NextResponse.json(
      { error: error.message || "Failed to generate LinkedIn headline" },
      { status: 500 }
    );
  }
}
